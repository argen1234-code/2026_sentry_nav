...
{
  RCLCPP_INFO(get_logger(), "Shutting down node, sending zero velocity command...");
  ...
  if (serial_driver_->port()->is_open()) {
    try {
      if (use_sbus_protocol_) {
        ...
        // 保持固定通道的值
        sbus_frame.channels[4] = 1000;
        sbus_frame.channels[5] = 1600;
        ...
        RCLCPP_INFO(get_logger(), "Zero velocity SBUS command sent (3 times)");
      } else {
        // 原始协议：发送零速度
        ...
        RCLCPP_INFO(get_logger(), "Zero velocity command sent (3 times)");
      }
    } catch (const std::exception & ex) {
      RCLCPP_ERROR(get_logger(), "Error sending zero velocity command: %s", ex.what());
    }
  }
...
void StandardRobotPpRos2Node::createPublisher()
{
  ...
  robot_state_info_pub_ =
    this->create_publisher<sentry_interfaces::msg::RobotStateInfo>("serial/robot_state_info", 10);
  joint_state_pub_ =
  ...
  event_data_pub_ =
    this->create_publisher<sentry_interfaces::msg::EventData>("referee/event_data", 10);
  all_robot_hp_pub_ =
    this->create_publisher<sentry_interfaces::msg::GameRobotHP>("referee/all_robot_hp", 10);
  game_status_pub_ =
    this->create_publisher<sentry_interfaces::msg::GameStatus>("referee/game_status", 10);
  ground_robot_position_pub_ = this->create_publisher<sentry_interfaces::msg::GroundRobotPosition>(
    "referee/ground_robot_position", 10);
  rfid_status_pub_ =
    this->create_publisher<sentry_interfaces::msg::RfidStatus>("referee/rfid_status", 10);
  robot_status_pub_ =
    this->create_publisher<sentry_interfaces::msg::RobotStatus>("referee/robot_status", 10);
  buff_pub_ = this->create_publisher<sentry_interfaces::msg::Buff>("referee/buff", 10);
}
...
void StandardRobotPpRos2Node::createSubscription()
{
  ...
  cmd_tracking_sub_ = this->create_subscription<sentry_interfaces::msg::Target>(
    "tracker/target", 10,
...
void StandardRobotPpRos2Node::getParams()
{
  ...
  // If using SBUS protocol, override serial port parameters
  if (use_sbus_protocol_) {
    RCLCPP_INFO(get_logger(), "Using SBUS protocol with 115200 baud (standard rate)");
    baud_rate = 115200;  // 使用标准波特率115200代替非标准的100000
    pt = Parity::EVEN;
    sb = StopBits::TWO;
...
{
  sentry_interfaces::msg::RobotStateInfo msg;
...
{
  sentry_interfaces::msg::EventData msg;
...
{
  sentry_interfaces::msg::GameRobotHP msg;
...
{
  sentry_interfaces::msg::GameStatus msg;
  ...
  if (record_rosbag_ && game_status.data.game_progress != previous_game_progress_) {
    ...
    switch (game_status.data.game_progress) {
      case sentry_interfaces::msg::GameStatus::COUNT_DOWN:
        service_name = "start_recording";
        break;
      case sentry_interfaces::msg::GameStatus::GAME_OVER:
...
{
  sentry_interfaces::msg::GroundRobotPosition msg;
...
{
  sentry_interfaces::msg::RfidStatus msg;
...
{
  sentry_interfaces::msg::RobotStatus msg;
...
{
  sentry_interfaces::msg::Buff msg;
...
void StandardRobotPpRos2Node::sendDataSbus()
{
  ...
  while (rclcpp::ok()) {
    ...
    try {
      // 根据用户反馈的最终映射：
      // 通道0 → 旋转（wz），让车子保持正前方向前移动
      // 通道2 → 前进后退（vx），前进时 > 1024
      // 通道3 → 左右移动（vy），右移时 > 1024
      // 通道4 → 固定为1000
      // 通道5 → 固定为1600
      // 有效范围：[600, 1007] 或 [1040, 1500]，避免死区 [1008, 1039]
      
      // 死区避免函数：限制在边界范围内
      auto avoidDeadZone = [](uint16_t value) -> uint16_t {
        if (value < 1024) {
          // 负值区域：限制在 [600, 1007]
          if (value < 600) return 600;
          if (value > 990) return 990;
          return value;
        } else if (value > 1024) {
          // 正值区域：限制在 [1040, 1500]
          if (value < 1060) return 1060;
      ...
      sbus_frame.channels[1] = SBUS_CENTER_VALUE;  //固定为1024
      
      // 通道0（wz旋转）：避开死区
      uint16_t ch0_value = SbusEncoder::mapValueToChannel(
        send_robot_cmd_data_.data.speed_vector.wz, max_angular_velocity_);
      sbus_frame.channels[0] = avoidDeadZone(ch0_value);
      
      // 通道2（vx前进后退）：避开死区
      uint16_t ch2_value = SbusEncoder::mapValueToChannel(
        send_robot_cmd_data_.data.speed_vector.vx, max_linear_velocity_);
      sbus_frame.channels[2] = avoidDeadZone(ch2_value);
      
      // 通道3（vy左右移动）：避开死区
      uint16_t ch3_value = SbusEncoder::mapValueToChannel(
        send_robot_cmd_data_.data.speed_vector.vy, max_linear_velocity_);
      sbus_frame.channels[3] = avoidDeadZone(ch3_value);
      
      sbus_frame.channels[4] = 1000;  // 固定为1000
      sbus_frame.channels[5] = 1600;  // 固定为1600
      // Encode SBUS frame
      std::vector<uint8_t> sbus_data = SbusEncoder::encode(sbus_frame);
      // 每次都打印速度和SBUS数据（合并在一行）
      std::string hex_str;
      for (size_t i = 0; i < std::min(size_t(10), sbus_data.size()); ++i) {
        char buf[8];
        snprintf(buf, sizeof(buf), "%02X ", sbus_data[i]);
        hex_str += buf;
      }
      
      RCLCPP_INFO(
        get_logger(),
        "[速度] vx=%.3f vy=%.3f wz=%.3f | [通道] CH0(wz)=%d CH2(vx)=%d CH3(vy)=%d CH4=%d CH5=%d | [SBUS] %s",
        ...
        sbus_frame.channels[0],
        sbus_frame.channels[2],
        sbus_frame.channels[3],
        sbus_frame.channels[4],
        sbus_frame.channels[5],
        hex_str.c_str()
      );
      
      // SBUS signal inversion (uncomment if hardware doesn't support inversion)
    ...
    // SBUS standard transmission rate ~100Hz (10ms)
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }
}
...
void StandardRobotPpRos2Node::visionTargetCallback(
  const sentry_interfaces::msg::Target::SharedPtr msg)
{
