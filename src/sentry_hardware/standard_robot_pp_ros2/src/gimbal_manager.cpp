...
: Node("gimbal_manager", options)
{
  ...
  cmd_sub_ = this->create_subscription<sentry_interfaces::msg::GimbalCmd>(
    "cmd_gimbal", 10,
...
void GimbalManagerNode::gimbalCmdCallback(const sentry_interfaces::msg::GimbalCmd::SharedPtr msg)
{
  static auto last_msg = std::make_shared<sentry_interfaces::msg::GimbalCmd>();
  ...
  *last_msg = *msg;
  ...
  if (msg->pitch_type == sentry_interfaces::msg::GimbalCmd::ABSOLUTE_ANGLE) {
    state_.pitch = msg->position.pitch;
    state_.pitch_ctrl.mode = ControlMode::POSITION;
  } else if (msg->pitch_type == sentry_interfaces::msg::GimbalCmd::VELOCITY) {
    state_.pitch_ctrl = {
  ...
  if (msg->yaw_type == sentry_interfaces::msg::GimbalCmd::ABSOLUTE_ANGLE) {
    state_.yaw = msg->position.yaw;
    state_.yaw_ctrl.mode = ControlMode::POSITION;
  } else if (msg->yaw_type == sentry_interfaces::msg::GimbalCmd::VELOCITY) {
    state_.yaw_ctrl = {
      .mode = ControlMode::VELOCITY,
