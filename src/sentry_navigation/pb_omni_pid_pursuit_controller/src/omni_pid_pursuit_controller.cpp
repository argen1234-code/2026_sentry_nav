  ...
  nav2_core::GoalChecker * /*goal_checker*/)
{
  ...
  // theta_dist: lookahead 点在 base_frame（gimbal_yaw_fake，全局固定方向）坐标系下的方向角
  double theta_dist = atan2(carrot_pose.pose.position.y, carrot_pose.pose.position.x);

  // 获取底盘真实朝向：查询 gimbal_yaw 在 gimbal_yaw_fake（base_frame）坐标系下的 yaw 角
  // fake_vel_transform 发布的 TF: gimbal_yaw -> gimbal_yaw_fake, rotation = RPY(0,0,-chassis_yaw)
  // 所以 gimbal_yaw_fake -> gimbal_yaw 的 yaw = chassis_yaw（底盘当前偏角）
  double chassis_yaw = 0.0;
  try {
    auto tf_stamped = tf_->lookupTransform(
      costmap_ros_->getBaseFrameID(), "gimbal_yaw",
      rclcpp::Time(0), transform_tolerance_);
    chassis_yaw = tf2::getYaw(tf_stamped.transform.rotation);
  } catch (tf2::TransformException & ex) {
    RCLCPP_WARN_THROTTLE(logger_, *(clock_), 1000,
      "Failed to get chassis yaw: %s. Using 0.", ex.what());
  }

  // 前进方向与底盘当前朝向的差角（需要旋转多少才能让车头对准前进方向）
  double heading_error = theta_dist - chassis_yaw;
  // 归一化到 [-pi, pi]
  while (heading_error > M_PI) heading_error -= 2.0 * M_PI;
  while (heading_error < -M_PI) heading_error += 2.0 * M_PI;

  // 当偏角超过阈值时，先原地转向；低于阈值后恢复全向运动同时持续修正朝向
  ...
  // 用 heading_error 作为角速度控制目标，让车头始终朝向运动方向
  auto angular_vel = enable_rotation_ ? heading_pid_->calculate(heading_error, 0) : 0.0;

  // 根据角度误差大小缩放角速度，避免小误差时仍以大速度旋转
  // 当误差小于10度(0.175 rad)时，线性缩放角速度
  const double angular_scaling_threshold = 0.175;  // 10度
  if (enable_rotation_ && fabs(heading_error) < angular_scaling_threshold) {
    double scaling_factor = fabs(heading_error) / angular_scaling_threshold;
    // 确保最小缩放因子为0.1，避免完全停止旋转
    scaling_factor = std::max(0.1, scaling_factor);
    angular_vel *= scaling_factor;
  }

  // 如果正在转向，强制线速度为0；否则全向运动同时修正朝向
  ...
  if (!isCollisionDetected(costmap_frame_local_plan)) {
    // 偏角超阈值时只转动；低于阈值时全向运动（x/y）同时持续修正朝向（w）
    cmd_vel.twist.linear.x = lin_vel * cos(theta_dist);
    cmd_vel.twist.linear.y = lin_vel * sin(theta_dist);
    cmd_vel.twist.angular.z = angular_vel;
  } else {
    // When collision is detected, send zero velocity command to stop the robot
    RCLCPP_WARN_THROTTLE(
      logger_, *(clock_), 1000,
      "Collision detected in the trajectory. Sending zero velocity command!");
